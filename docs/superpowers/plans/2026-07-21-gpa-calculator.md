            ...
            {fields.map((field, index) => (
              <div key={field.id} className="flex flex-wrap items-end gap-2">
                <div className="min-w-[8rem] flex-1">
                  <label
                    htmlFor={`${fieldIdPrefix}-course-${index}-name`}
                ...
                <div className="w-24">
                  <label
                    htmlFor={`${fieldIdPrefix}-course-${index}-credits`}
                    className="block text-xs text-text-grey"
                  >
                    単位数
                  </label>
                ...
                {scale.method === "grade" ? (
                  <div className="w-32">
                    <label
                      htmlFor={`${fieldIdPrefix}-course-${index}-grade`}
                      className="block text-xs text-text-grey"
                    >
                      成績
                    </label>
                  ...
                  <div className="w-32">
                    <label
                      htmlFor={`${fieldIdPrefix}-course-${index}-score`}
                      className="block text-xs text-text-grey"
                    >
                      素点（0〜100）
                    </label>
...
export const metadata: Metadata = {
  ...
  // （/guide 配下は app/guide/layout.tsx が title を上書きしてテンプレートを
  //   打ち消しているため二重にならない。/gpa は直下なのでテンプレートが効く）
  title: "GPA計算機｜大学別の換算方式に対応（京大・阪大・早慶ほか）",
  description:
