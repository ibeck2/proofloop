...
✅ **確認済み（2026-08-08 実測）：Node v24 は `.ts` を直接 import できる**（型ストリップが既定で有効）。`npx tsx` も追加依存も不要で、`node` でそのまま動く。
実行時に `MODULE_TYPELESS_PACKAGE_JSON` の警告が出るが無害。**ロジックをスクリプト側に複製してはいけない**（片方だけ直す事故が起きる）。
...
- Produces: `get_claim_preview(uuid)` / `apply_for_claim(uuid,text,text)` / `decide_claim(uuid,text,text,text,text)` / `revoke_claim(uuid,text)` / `submit_dispute(uuid,text,text,text)` / `resolve_dispute(uuid,text,text)`

> ⚠️ **実装後にレビュー指摘で7件修正した（コミット `5aed579`）。以下のSQLは初版であり、現物は `supabase/migrations/029_org_claim_rpc.sql` が正。**
> 主な変更：①スナップショット復元を `jsonb_populate_record` で全列に拡張 ②`decide_claim` が団体の現在状態を再検査 ③`resolve_dispute` 認容時にオーナー行を確実に削除し `revoke_claim` の戻り値を検査 ④`revoke_claim` は `frozen` の `claim_status` を落とさない ⑤`apply_for_claim` は別ユーザーの先行申請を上書きしない ⑥`decide_claim` から `p_signals` を削除し `p_verdict` を追加 ⑦`ON CONFLICT DO NOTHING` → `DO UPDATE`
...
✅ **確認済み（2026-08-08）：`app/login/page.tsx` の遷移は2箇所ある。`:85` の `router.push("/mypage")`（学生・団体）と `:109` の `router.push("/companydashboard")`（企業）。**
**`:85` 側にだけ**次を挟む。企業アカウントは団体を claim しないため `:109` は触らない。
...
✅ **確認済み（2026-08-08）：`Button` の `variant` は `primary` / `secondary` / `outline` / `outlineMuted` / `ghost`。**
ただし **`secondary` は深紅（seal）地**なので使わないこと。一覧に何枚もカードが並ぶ画面で深紅ボタンを複数置くと、
CLAUDE.md §3 の「深紅は静止状態で1画面2箇所まで」を破る。

**`app/admin/reviews/page.tsx:233,242` の前例に合わせ、`primary`（紺）＋ `outlineMuted`（白地・罫線）だけを使う。**
Task 5・6・7 のコード中の副次ボタンはすべて `outlineMuted` を指定済み。
...
- `decide_claim` の引数は `(uuid, text, text, text, text)`。第5引数は `p_verdict`（`green` / `red`）で、Task 5 が `resolveVerdict(evaluateSignals(row.signals))` の結果を渡す。※初版は `p_signals jsonb` だったが、admin が収集済みの事実を上書きできてしまうためレビュー指摘で変更した。
