...
- `supabase/migrations/067_schedule_poll_responses.sql` — `schedule_poll_responses`テーブル・RLS・`submit_schedule_poll_response`RPC
- `supabase/migrations/068_schedule_poll_views.sql` — `schedule_poll_views`テーブル・RLS
- `supabase/migrations/069_schedule_poll_decide.sql` — `decide_schedule_poll_candidate`RPC
...
**Files:**
- Create: `supabase/migrations/067_schedule_poll_responses.sql`
...
```sql
-- 067 schedule_poll_responses: 候補日時への回答（○/△/×）
--
...
```bash
git add supabase/migrations/067_schedule_poll_responses.sql
...
**Files:**
- Create: `supabase/migrations/068_schedule_poll_views.sql`
...
```sql
-- 068 schedule_poll_views: 日程調整の既読記録（初回閲覧時刻のみ）
--
-- INSERT ... ON CONFLICT DO NOTHING はSET句が無いためUPDATE権限を必要としない
-- （067のresponsesと異なり、ここはRPCを使わずテーブル直接INSERTで安全に書ける）。
...
```bash
git add supabase/migrations/068_schedule_poll_views.sql
...
**Files:**
- Create: `supabase/migrations/069_schedule_poll_decide.sql`
...
```sql
-- 069 decide_schedule_poll_candidate: 幹事による確定操作
--
...
```bash
git add supabase/migrations/069_schedule_poll_decide.sql
