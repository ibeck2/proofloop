...
const TASKS = [
  ...
  ["B1", 2, "claim動線の設計＋Plan Mode承認（スキーマ変更含む）", "Claude起案→オーナー承認", "—", "承認済み設計書", 1, 2, "未着手"],
  ["B2", 2, "claim動線の実装・テスト・本番反映", "Claude", "B1", "掲載ページから団体が自分で引き取れる", 2, 3, "未着手"],
  ["B3", 2, "claimトークンの一括発行（2,354件）", "Claude", "B2", "団体別のclaim URL一覧", 3, 3, "未着手"],
  ["B4", 2, "声かけ優先順位ルールの確定", "オーナー＋Claude", "—", "スコア定義（到達30/大学25/需要25/規模20）の承認", 1, 1, "✅ 草案あり"],
  ["B5", 2, "優先順位付きリストの生成・バッチ分割", "Claude", "B4", "団体_声かけ優先順位.csv（2,354件・バッチ1〜6）", 1, 1, "✅ 完了"],
  ...
  ["D3", 4, "middleware.ts でサーバーサイド認証ゲート（リスクS1）", "Claude", "—", "/admin をサーバー側で保護", 3, 4, "未着手"],
...
// ============================================================
{
  ...
  title(ws, "ProofLoop タスクシート 2026-08 → 2027-01");
