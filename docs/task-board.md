...
| AG | **スマホ対応の網羅監査**（本ブランチでクラブ管理サイドバーのハンバーガー未実装・`/for-clubs`はみ出しの既知バグ2件は先行修正済み） | 🟡 監査完了・追加で見つかった3件は修正待ち | 監査結果とまとめは`docs/superpowers/specs/2026-08-21-mobile-audit-findings.md`。要修正3件（`/baito/simulator`ラベル重なり・`/for-clubs`本文はみ出し・`/clubprofile`ヘッダー二重表示＋固定ボトムナビ誤表示）＋軽微1件（`/clubfinance`のファイル未選択ラベル省略）。実機確認は基準の375pxでなく約487px幅で行われた点に留意（詳細は監査結果の「まとめ」冒頭）。次は結果に基づく修正プランを別途立てる |
