...
| I2 | **claim 動線・公開前の残作業**（下記） | 🔴 新規 | マイグレーション033の本番適用＝**オーナー承認待ち**。トークン第1バッチ送信のゲート |
...
1. **マイグレーション033の本番適用（オーナー承認事項）。** claim動線の Critical 3件を塞ぐ。
   `BEGIN;...ROLLBACK;` 検証済み。**うち1件（C3）は claim と無関係に、いま本番で
   新規登録とプロフィール保存を壊している**（030 の副作用。`profiles` の upsert が
   `permission denied`）。claim動線の公開可否と切り離して、これだけでも先に当てる価値がある。
