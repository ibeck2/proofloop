...
1. **Critical**：Task 8の実機確認（ボタン幅104px→120px）が、Webフォント読み込み前のフォールバックフォントで測定してしまっており、実際のNoto Sans JPでは「ガントチャート」ラベルが依然として親要素の`overflow-hidden`で切れていた。固定幅指定をやめ、CSS Grid（`inline-grid grid-flow-col auto-cols-fr`）による自動均等幅に変更して解消。
2. **Important**：ボタン列コンテナが`shrink-0`のままだと`overflow-x-auto`が実質機能せず、タブレット幅（640〜820px）でページ全体に横スクロールが波及しうる状態だった。`min-w-0`に変更して解消。
...
- ライブQA（ブラウザでの実機確認）はオーナーにログインしていただいたうえで実施済み（表のデフォルト表示・4ビュー切替・種別カラーの一貫性・モーダル入力欄の余白・カンバンチップ行を確認）。ただし375px幅でのスクリーンショットはツールの制約で取得できておらず、次に触る際に実機で確認すること。
