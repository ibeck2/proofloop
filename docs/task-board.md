...
| AE | **`/clubschedule` 日程調整機能（新規）**（タスクJのヒアリング機能候補） | ✅ **完了**（2026-08-20） | 詳細は下記セクションAE。レビューで見つかった認可まわりの穴4件はすべて本番修正済み。オーナーによる複数アカウントでの実機確認が残る |
...
### やったこと
- **データモデル**：`schedule_polls`／`schedule_poll_candidates`（候補日時＋確定フラグ）／`schedule_poll_responses`（○/△/×回答）／`schedule_poll_views`（既読記録）の4テーブル（マイグレーション066〜072、7ファイル）。回答・確定の書き込みはPostgRESTのupsertを使わず`submit_schedule_poll_response`・`decide_schedule_poll_candidate`のSECURITY DEFINER RPCに一本化（CLAUDE.mdの既知の落とし穴＝upsertがPK列のUPDATE権限を要求する問題を、そもそも該当経路を作らないことで回避）。
...
- **UI**：`/clubschedule`（一覧・新規作成）／`/clubschedule/[id]`（候補ごとの集計・回答マトリクス・既読状況・幹事による確定操作）。`ClubSidebar`にナビゲーション追加。
...
- **スコープ外**（将来拡張として記録）：確定候補の`/clubevents`イベントへの自動変換、`tasks`・`organization_members.title`との文脈紐付け、カレンダーグリッド型（When2meet型）の空き時間自由入力、cronによる自動リマインド（現状のリポジトリに定期実行基盤が無いため）。
...
- `/clubschedule`一覧の詳細リンクが`withOrgQuery`を使わず`?orgId=`を引き継がない（詳細ページ自体はpoll自身の所属団体でスコープするよう修正済みのため実害なし。URLの見た目上の不整合のみ残る）。
...
- Task9の実際のメール到達確認・複数アカウントでのシナリオ（未読→既読未回答の遷移、非admin向けの「確定」列非表示）はテスト用団体がメンバー1名のみだったため未検証。オーナーが実アカウント・複数メンバーで確認する価値がある。
