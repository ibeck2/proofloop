...
| D | Resend 独自ドメイン認証 | 🟡 DNS設定完了（2026-08-14）・Resend側検証待ち | 次はVerified確認 → コード側`from`をRESEND_FROM化 |
...
| I2 | **claim 動線・公開前の残作業**（下記） | ✅ **完了**（2026-08-13） | 「トークン第1バッチを送る前に必ず終えるもの」の1〜7全項目が完了。**トークン送付自体は依然ブロック中**（通知メール本体＝タスク9が未実装で、その前提のResend独自ドメイン認証＝タスクDが未完了のため） |
...
**次の推奨着手順：D（Verified確認）→ 団体ページdescription生成 → H**（Jは独立して並行進行）
...
- **D**＝DNS設定は完了。Resend側のVerified確認とコード側`from`差し替えが残作業
...
### 残作業
1. Resend側のステータスが作業終了時点で「Pending」だった。**Verified になったか確認する**（`https://resend.com/domains` を開き `proofloop.jp` のステータスを見る）
2. Verified確認後、コード側の`from`を`ProofLoop運営 <contact@proofloop.jp>`に差し替え（`RESEND_FROM`環境変数化）→ 実送信テスト
