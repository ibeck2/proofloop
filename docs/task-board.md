...
| I | **claim 動線の実装**（掲載団体が自分のページを引き取る） | 🔴 **新規・ハードブロッカー** | **スキーマ変更を含むため Plan Mode 承認が必須**。これが無いまま通知を送ると重複ページでSEOを毀損する |

**次の推奨着手順：G → I（設計・承認）→ D → 団体ページdescription生成 → H → I（実装）**
...
**承認済み2,421団体のうち、アカウントの主がいるのは1件だけ。SNS/サイトで到達できるのは2,354件（97.2%）。**
...
- 🔴 **claim 動線が未実装**（タスクI）。このまま通知を送ると重複ページができてSEOを毀損する。**通知の前に必ず実装する。**
