    ...
    <div className="bg-white text-primary min-h-screen font-body pb-20 md:pb-0">
      <main className="w-full max-w-[1200px] mx-auto px-6 py-12 md:py-20 flex flex-col gap-16">
        ...
        <nav className="flex items-center gap-2 text-xs text-text-grey -mb-10">
          ...
          <span className="material-symbols-outlined text-sm">chevron_right</span>
          <span className="text-primary font-bold">お金・奨学金どうする？</span>
        </nav>

        {/* Hero */}
        <section className="flex flex-col gap-6 max-w-3xl">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-accent text-2xl">savings</span>
            <span className="text-accent text-sm font-bold tracking-widest uppercase">Money Guide</span>
          </div>
        ...
        <section id="living-cost" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="text-primary text-xl md:text-2xl font-black">大学生の生活費はいくら？</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border border-[#f0f2f5]">
              <thead className="bg-primary/5">
                <tr>
                  <th className="px-4 py-3 text-left font-bold text-primary">項目</th>
                  <th className="px-4 py-3 text-left font-bold text-primary">自宅生（月）</th>
                  <th className="px-4 py-3 text-left font-bold text-primary">一人暮らし（月）</th>
                </tr>
              </thead>
              <tbody className="text-text-grey">
                {LIVING_COST_BREAKDOWN.map((row) => (
                  <tr key={row.label} className="border-t border-[#f0f2f5]">
                    <td className="px-4 py-3 font-bold text-primary">{row.label}</td>
                    <td className="px-4 py-3">{row.home}</td>
                    <td className="px-4 py-3">{row.alone}</td>
                  </tr>
                ))}
                <tr className="border-t border-[#f0f2f5] bg-accent/5">
                  <td className="px-4 py-3 font-black text-primary">合計</td>
                  <td className="px-4 py-3 font-black text-primary">約 3〜8万円</td>
                  <td className="px-4 py-3 font-black text-primary">約 10〜15万円</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="border border-emerald-200 bg-emerald-50 p-5 flex gap-3">
            ...
            <div className="flex-1">
              <p className="text-emerald-900 font-bold text-sm">不足分はバイトで補える？</p>
        ...
        <section id="overview" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="text-primary text-xl md:text-2xl font-black">奨学金の全体像（4つの提供元）</h2>
            <p className="text-text-grey text-sm">
            ...
            {SCHOLARSHIP_PROVIDERS.map((provider, i) => (
              <div key={i} className="border border-[#f0f2f5] p-5 flex flex-col gap-3">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 ${provider.bg} flex items-center justify-center shrink-0`}>
                    <span className={`material-symbols-outlined text-xl ${provider.color}`}>
                      {provider.icon}
                    </span>
                  </div>
                  <h3 className="font-black text-primary text-sm leading-tight">{provider.name}</h3>
                </div>
                <p className="text-text-grey text-sm leading-relaxed">{provider.feature}</p>
                <div className="flex gap-2 text-xs">
                  <span className="text-primary font-bold shrink-0">種類：</span>
                  <span className="text-text-grey leading-relaxed">{provider.types}</span>
                </div>
                <div className="flex gap-2 text-xs pt-2 border-t border-[#f0f2f5]">
                  <span className="text-accent font-bold shrink-0">→ Tips</span>
                  <span className="text-text-grey leading-relaxed">{provider.note}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border border-amber-200 bg-amber-50 p-5 flex gap-3">
            ...
            <div>
              <p className="text-amber-900 font-bold text-sm">「給付型」を装う有料サービスに注意</p>
        ...
        <section id="jasso" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="text-primary text-xl md:text-2xl font-black">JASSO奨学金（給付・第一種・第二種）</h2>
            <p className="text-text-grey text-sm">
          ...
          <div className="flex flex-col gap-4">
            {JASSO_DETAILS.map((item, i) => (
              <div key={i} className="border border-[#f0f2f5] p-6 flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 ${item.color} flex items-center justify-center shrink-0`}>
                    <span className="material-symbols-outlined text-white text-xl">{item.icon}</span>
                  </div>
                  <h3 className="font-black text-primary text-base">{item.name}</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                  <div className="bg-primary/5 p-3 flex flex-col gap-1">
                    <span className="text-text-grey font-bold">月額</span>
                    <span className="text-primary font-black text-sm">{item.monthly}</span>
                  </div>
                  <div className="bg-primary/5 p-3 flex flex-col gap-1">
                    <span className="text-text-grey font-bold">返済</span>
                    <span className="text-primary font-black text-sm">{item.repay}</span>
                  </div>
                  <div className="bg-primary/5 p-3 flex flex-col gap-1">
                    <span className="text-text-grey font-bold">主な基準</span>
                    <span className="text-primary font-black text-sm leading-tight">{item.criteria}</span>
                  </div>
                </div>
          ...
          <div className="border border-[#f0f2f5] p-6 flex flex-col gap-4">
            <h3 className="text-primary font-black text-base">申請時期：予約採用と在学採用</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-rose-600 text-lg">event_available</span>
                  <h4 className="text-primary font-bold text-sm">予約採用（高校在学中に申請）</h4>
                </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-primary font-bold text-sm">在学採用（大学入学後に申請）</h4>
                </div>
                <p className="text-text-grey text-xs leading-relaxed">
        ...
        <section id="private" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="text-primary text-xl md:text-2xl font-black">民間・自治体・大学独自奨学金の探し方</h2>
            <p className="text-text-grey text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              <div key={i} className="border border-[#f0f2f5] p-5 flex gap-4">
                <div className="w-10 h-10 bg-rose-50 flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-rose-600 text-xl">{item.icon}</span>
                </div>
                <div className="flex flex-col gap-1.5">
        ...
        <section id="repay" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="text-primary text-xl md:text-2xl font-black">奨学金返済の基本</h2>
            <p className="text-text-grey text-sm">
          ...
          {/* 返済シミュレーション例 */}
          <div className="border border-[#f0f2f5] p-6 flex flex-col gap-4">
            <h3 className="text-primary font-black text-base">返済シミュレーション例</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
              {[
                ...
                <div key={row.label} className="bg-primary/5 p-3 flex flex-col gap-1">
                  <span className="font-bold text-text-grey">{row.label}</span>
                  <span className="text-accent font-black text-base">{row.value}</span>
                  <span className="text-text-grey leading-snug">{row.sub}</span>
                </div>
              ))}
            </div>
            <p className="text-[11px] text-text-grey">
          ...
          <div className="border border-emerald-200 bg-emerald-50 p-5 flex gap-3">
            ...
            <div>
              <p className="text-emerald-900 font-bold text-sm">返済が苦しくなったら：減額返還・返還期限猶予</p>
              <p className="text-emerald-800 text-xs leading-relaxed mt-1">
        ...
        <section id="saving" className="flex flex-col gap-6 scroll-mt-20">
          <h2 className="text-primary text-xl md:text-2xl font-black">節約術・学割を活用する</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
