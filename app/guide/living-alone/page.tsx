...
const INITIAL_COSTS = [
  ...
  { label: "礼金", amount: "家賃0〜1か月分", Icon: Gift, note: "大家へのお礼で返還なし。ゼロ物件も増えている。" },
  { label: "仲介手数料", amount: "家賃0.5〜1か月分＋税", Icon: Building2, note: "不動産会社への手数料。半額・無料の会社もある。" },
  { label: "前家賃", amount: "家賃1か月分", Icon: CalendarDays, note: "入居する月の家賃を前払い。" },
  { label: "火災保険料", amount: "1.5〜2万円（2年）", Icon: Flame, note: "加入必須のことが多い。自分で安い保険を選べる場合も。" },
  { label: "保証会社利用料", amount: "家賃0.5〜1か月分", Icon: ShieldCheck, note: "連帯保証人の代わり。近年はほぼ必須。" },
  { label: "鍵交換費用", amount: "1.5〜2万円", Icon: Key, note: "防犯のための鍵交換。" },
  { label: "家具・家電", amount: "10〜20万円", Icon: Armchair, note: "冷蔵庫・洗濯機・電子レンジ・ベッド等。実家からの転用で圧縮可。" },
  { label: "引越し費用", amount: "3〜8万円", Icon: Truck, note: "距離と時期で変動。3〜4月の繁忙期は高い。" },
] as const;
...
const FOOD_TIERS = [
  { label: "外食・コンビニ中心", value: "月 4〜5万円", Icon: Store },
  { label: "自炊と外食の併用", value: "月 2〜3万円", Icon: UtensilsCrossed },
  { label: "ほぼ自炊・作り置き", value: "月 1〜1.5万円", Icon: ChefHat },
] as const;
    ...
    <div className="bg-paper text-ink min-h-screen font-body pb-20 md:pb-0">
      <main className="w-full max-w-[1200px] mx-auto px-6 py-12 md:py-20 flex flex-col gap-16">
        ...
        <nav className="flex items-center gap-2 text-xs text-graphite -mb-10">
          ...
          <ChevronRight className="w-4 h-4" aria-hidden="true" />
          <span className="text-ink font-bold">一人暮らしどうする？</span>
        </nav>

        {/* Hero */}
        <section className="flex flex-col gap-6 max-w-3xl">
          <div className="flex items-center gap-2">
            <House className="w-6 h-6 text-ink" aria-hidden="true" />
            <span className="text-ink text-sm font-bold tracking-widest uppercase">Living Alone Guide</span>
          </div>
        ...
        <section id="cost" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">一人暮らしにかかるお金の全体像</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-rule p-6 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <Receipt className="w-5 h-5 text-ink" aria-hidden="true" />
                <h3 className="text-ink font-black text-base">初期費用（入居時に一度）</h3>
              </div>
              <p className="text-ink font-numeric tabular-nums font-black text-2xl">約 30〜50万円</p>
            ...
            <div className="border border-rule p-6 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-ink font-black text-base">毎月の生活費</h3>
              </div>
              <p className="text-ink font-numeric tabular-nums font-black text-2xl">約 10〜15万円</p>
              <p className="text-graphite text-xs leading-relaxed">
          ...
          <div className="border border-rule border-l-4 border-l-ink bg-mist p-5 flex gap-3">
            <PiggyBank className="w-[18px] h-[18px] text-ink shrink-0 mt-0.5" aria-hidden="true" />
            <div className="flex-1">
              <p className="text-ink font-bold text-sm">この費用、どうやって用意する？</p>
              <p className="text-graphite text-xs leading-relaxed mt-1">
                奨学金の種類・申請時期・生活費の内訳は「お金・奨学金ガイド」で詳しく解説しています。
                <Link href="/guide/money" className="text-ink font-bold underline ml-1">
                  お金・奨学金どうする？ →
                </Link>
              </p>
            </div>
        ...
        <section id="initial" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">初期費用の内訳</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {INITIAL_COSTS.map((item) => (
              <div key={item.label} className="border border-rule p-5 flex gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <div className="flex items-baseline gap-2 flex-wrap">
                    ...
                    <span className="text-ink font-numeric tabular-nums text-xs font-black">{item.amount}</span>
                  </div>
          ...
          <div className="border border-rule border-l-4 border-l-ink bg-mist p-5 flex gap-3">
            ...
            <div>
              <p className="text-ink font-bold text-sm">初期費用を抑える4つのコツ</p>
              <p className="text-graphite text-xs leading-relaxed mt-1">
        ...
        <section id="rent" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">家賃の相場とエリア選び</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border border-rule">
              <thead className="bg-mist">
                <tr>
                  <th className="px-4 py-3 text-left font-bold text-ink">エリア</th>
                  <th className="px-4 py-3 text-left font-bold text-ink">家賃の目安（月）</th>
                  <th className="px-4 py-3 text-left font-bold text-ink hidden md:table-cell">ポイント</th>
                </tr>
              </thead>
              <tbody className="text-graphite">
                {RENT_BY_AREA.map((row) => (
                  <tr key={row.area} className="border-t border-rule">
                    <td className="px-4 py-3 font-bold text-ink">{row.area}</td>
                    <td className="px-4 py-3 font-numeric tabular-nums font-black text-ink whitespace-nowrap">{row.range}</td>
          ...
          <div className="border border-rule p-6 flex flex-col gap-3">
            <h3 className="text-ink font-black text-base">女子の部屋選びは防犯を最優先に</h3>
        ...
        <section id="food" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">食費と自炊</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {FOOD_TIERS.map((row) => (
              <div key={row.label} className="border border-rule p-5 flex flex-col gap-2 items-start">
                <row.Icon className="w-6 h-6 text-ink" aria-hidden="true" />
                <h3 className="text-ink text-sm font-bold">{row.label}</h3>
                <p className="text-ink font-numeric tabular-nums font-black text-lg">{row.value}</p>
              </div>
            ))}
          </div>
        ...
        <section id="checklist" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">一人暮らしで必要なものチェックリスト</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {CHECKLIST.map((group) => (
              <div key={group.category} className="border border-rule p-5 flex flex-col gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-mist flex items-center justify-center shrink-0">
                    <group.Icon className="w-5 h-5 text-ink" aria-hidden="true" />
                  </div>
                  <h3 className="font-black text-ink text-sm">{group.category}</h3>
                </div>
                <div className="flex flex-col gap-2">
                  <span className="text-[11px] font-bold text-ink">入学までに揃える</span>
                  <div className="flex flex-wrap gap-1.5">
                    {group.must.map((item) => (
                      <span key={item} className="text-xs px-2 py-1 bg-mist text-ink border border-rule">
                        {item}
                      </span>
                    ))}
                  </div>
                  <span className="text-[11px] font-bold text-graphite mt-1">後から買い足す</span>
                  <div className="flex flex-wrap gap-1.5">
                    {group.later.map((item) => (
                      <span key={item} className="text-xs px-2 py-1 bg-mist text-graphite border border-rule">
                        {item}
                      </span>
                    ))}
                  </div>
        ...
        <section id="income" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">仕送りとバイトでどう賄う？</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-rule p-6 flex flex-col gap-2">
              <h3 className="text-ink font-black text-base">仕送りの平均額</h3>
              <p className="text-ink font-numeric tabular-nums font-black text-2xl">月 約7〜8万円</p>
              <p className="text-graphite text-xs leading-relaxed">
                全国大学生協連の調査では一人暮らし学生への仕送りは月7万円前後。家賃を仕送りで、生活費をバイトで賄うのが典型的なモデルです。
              </p>
            </div>
            <div className="border border-rule p-6 flex flex-col gap-2">
              <h3 className="text-ink font-black text-base">仕送りなしのモデルケース</h3>
              <p className="text-graphite text-sm leading-relaxed">
          ...
          <div className="border border-rule border-l-4 border-l-ink bg-mist p-5 flex gap-3">
            <Lightbulb className="w-[18px] h-[18px] text-ink shrink-0 mt-0.5" aria-hidden="true" />
            <div className="flex-1">
              <p className="text-ink font-bold text-sm">バイトでいくら稼げば足りる？</p>
              <p className="text-graphite text-xs leading-relaxed mt-1">
        ...
        <section id="tough" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">「一人暮らしがきつい」と感じたら</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {TOUGH_CARDS.map((item) => (
              <div key={item.title} className="border border-rule p-5 flex flex-col gap-2">
                <item.Icon className="w-6 h-6 text-ink" aria-hidden="true" />
                <h3 className="text-ink text-sm font-bold">{item.title}</h3>
                <p className="text-graphite text-xs leading-relaxed">{item.body}</p>
              </div>
            ))}
          </div>
          ...
          {/* サークル送客（補足） */}
          <div className="border border-rule border-l-4 border-l-ink bg-mist p-5 flex gap-3">
            <Users className="w-[18px] h-[18px] text-ink shrink-0 mt-0.5" aria-hidden="true" />
            <div className="flex-1">
        ...
        <section id="admin" className="flex flex-col gap-6 scroll-mt-20">
          <div className="flex flex-col gap-1">
            <h2 className="font-mincho text-ink text-xl md:text-2xl font-black">手続きミニ知識（住民票・世帯主）</h2>
            <p className="text-graphite text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="border border-rule p-5 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <FileText className="w-5 h-5 text-ink" aria-hidden="true" />
                <h3 className="text-ink text-sm font-bold">住民票は移すべき？</h3>
              </div>
              <p className="text-graphite text-xs leading-relaxed">
            ...
            <div className="border border-rule p-5 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <IdCard className="w-5 h-5 text-ink" aria-hidden="true" />
                <h3 className="text-ink text-sm font-bold">世帯主はどうなる？</h3>
              </div>
              <p className="text-graphite text-xs leading-relaxed">
        ...
        <section className="border border-rule p-6 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href="/guide/money"
              className="flex items-center gap-2 px-4 py-2 border border-rule hover:border-ink/40 text-sm font-bold text-ink hover:text-ink transition-colors"
            >
