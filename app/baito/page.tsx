...
function JobCard({ job }: { job: JobListing }) {
  const wageLabel =
    job.hourly_wage_min && job.hourly_wage_max
      ? `¥${job.hourly_wage_min.toLocaleString()}〜${job.hourly_wage_max.toLocaleString()}/h`
      : job.hourly_wage_min
      ? `¥${job.hourly_wage_min.toLocaleString()}/h〜`
      : null;
  ...
  return (
    <article className="bg-white border border-[#f0f2f5] hover:border-accent/40 hover:shadow-sm transition-all flex flex-col">
      {/* ヘッダー */}
      <div className="p-5 border-b border-[#f0f2f5] flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1.5">
          <span className={`text-xs font-bold px-2 py-0.5 self-start ${typeBg}`}>
            {typeLabel}
          </span>
          <h3 className="text-primary text-base font-bold leading-snug">{job.title}</h3>
          {job.company_name && (
            <p className="text-text-grey text-xs">{job.company_name}</p>
          )}
        </div>
        {wageLabel && (
          <div className="text-right shrink-0">
            <p className="text-accent font-black text-lg leading-none">{wageLabel}</p>
          </div>
        )}
      </div>
      ...
      <div className="p-5 flex flex-col gap-4 flex-1">
        ...
        {/* 説明 */}
        {job.description && (
          <p className="text-text-grey text-sm leading-relaxed line-clamp-3">
            {job.description}
          </p>
        )}
      ...
      <div className="p-5 pt-0">
        ...
        <p className="text-[10px] text-text-grey text-center mt-1.5">
          ※外部サービスへ移動します（PR）
        </p>
      </div>
    </article>
